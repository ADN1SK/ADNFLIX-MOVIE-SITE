...
import { Search, LayoutDashboard, User, Moon, Sun, Menu, X } from "lucide-react";
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsLoading(true);
      try {
        const res = await fetch(`/api/movies/search/movie?query=${encodeURIComponent(searchQuery)}`);
  ...
  return (
    ...
    >
      <div className="max-w-screen-2xl mx-auto w-full flex items-center gap-4 md:gap-8">
        ...
        <Link 
          to="/dashboard"
          className="hidden lg:flex items-center gap-2 px-4 py-2 rounded-lg bg-card-bg shadow-skeuo-sm border border-text-main/5 hover:shadow-skeuo-md hover:border-primary/30 transition-all text-sm font-medium shrink-0"
        >
        ...
        <div className="flex-1 flex justify-center max-w-2xl mx-auto relative">
          ...
          >
            <div className={cn(
              ...
              isSearchExpanded ? "shadow-skeuo-inner border-primary/40" : "group-hover:border-text-main/20"
            )} />
